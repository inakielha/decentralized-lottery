    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("enterRaffle", function () {
              it("reverts when you don't pay enough", async () => {
                  await expect(raffle.enterRaffle()).to.be.revertedWithCustomError( // is reverted when not paid enough or raffle is not open
                  raffleContract,"Raffle_NotEnoghETHEnter"
                  )
              })
              ...
              it("doesn't allow entrance when raffle is calculating", async () => {
                  ...
                  await expect(raffle.enterRaffle({ value: raffleEntranceFee })).to.be.revertedWithCustomError( // is reverted as raffle is calculating
                      raffleContract,"Raffle__NotOpen"
                  )
              })
          })
          ...
          describe("performUpkeep", function () {
              ...
              it("reverts if checkup is false", async () => {
                  await expect(raffle.performUpkeep("0x")).to.be.revertedWithCustomError( 
                      raffleContract,"Raffle__UpkeepNotNeeded"
                  )
              })
